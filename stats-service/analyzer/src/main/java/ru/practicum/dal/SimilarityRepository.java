...
public interface SimilarityRepository extends JpaRepository<EventSimilarity, Long> {
    ...
    @Query(value = """
            select event_b_id, score from event_similarity 
            where event_a_id = :event and event_b_id in
            (select event_id from user_actions where user_id <> :userId and event_id <> :event)
            order by score desc
    ...
    @Query(value = """
            ...
            select event_b_id as eventId, max(score) as score from event_similarity
